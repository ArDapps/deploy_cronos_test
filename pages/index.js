...
export default function Home() {
    ...
    const pro = new Web3.providers.HttpProvider("https://cronos-testnet-3.crypto.org:8545");
...
//Get The ABI
           ...
           const nftNetWorkObject =  convertNftContratFileToJson.networks[338];
           const nftMarketWorkObject =  convertMarketContractFileToJson.networks[338];
           console.log("sjdhisdbfishdbv")

           if(nftMarketWorkObject && nftMarketWorkObject){

            const nftAddress = await nftNetWorkObject.address;

            setNFtAddress(nftAddress)
            const marketAddress = nftMarketWorkObject.address;
            setMarketAddress(marketAddress)

            let web33web = new Web3(pro)

            const deployedNftContract =  new web33web.eth.Contract(nFTAbi,nftAddress);

            const deployedNftContractWithMetaMAsk = await new web3Api.web3.eth.Contract(nFTAbi,nftAddress);
            setNFtContract(deployedNftContractWithMetaMAsk)
            

            const deployedMarketContract =  new web33web.eth.Contract(markrtAbi,marketAddress);
            const deployedMarketContractMEtamask = await new web3Api.web3.eth.Contract(markrtAbi,marketAddress);

            setMarketContract(deployedMarketContractMEtamask)
            console.log("***********deployedMarketContract *****",deployedMarketContract)
          
            try{
                   //Fetch all unsold items
            let data =  await deployedMarketContract.methods.getAllUnsoldItems().call()
            console.log(data)
            console.log("***********THERE *****")
...
//TODO: fix this object
            ...
            return myItem;
              ...
              console.log(e);
               ...
               window.alert("You are at Wrong Network, Connect with Polygon Please")
           }
...
const buyNFT = async (nftItem)=>{
    ...
    router.reload()
     console.log(result)
      ...
      window.alert("You should be in the Polygon Mainnet in Metamask")
    }
    }catch(e){
      window.alert("You should be in the Polygon Mainnet in Metamask")
